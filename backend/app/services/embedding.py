...
class SBERTModel(EmbeddingModelInterface):
    ...
    def embed(self, chunks: List[ChunkInput]) -> List[List[float]]:
        ...
        """
        This condition is just a safety check because if some images end up here somehow the model
        doesn't crash because it was given images instead of text.
        """
        if not texts:
            return []
...
class EmbeddingService:
    ...
    # This is the function that the controller actually sees, it gives the input here
    def process_embeddings(self, model_name: str, chunks: List[ChunkInput]) -> EmbeddingResponse:
        ...
        for i, vector in enumerate(vectors):
            # Pair the vector back with its metadata
