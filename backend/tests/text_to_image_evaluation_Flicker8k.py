...
def parse_captions_file(filepath):
    """
    Parses the captions text file. 
    Adjust the split logic based on your exact captions.txt format.
    Common formats handled here:
    1. image_name.jpg#0\tA boy smiles... (Flickr style)
    2. image_name.jpg,A boy smiles... (CSV style)
    """
    dataset = []
    with open(filepath, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            # Extract filename and caption based on common delimiters
            if '\t' in line:
                file_part, caption = line.split('\t', 1)
                filename = file_part.split('#')[0] # Clean things like .jpg#0
            elif ',' in line:
                filename, caption = line.split(',', 1)
            else:
                # Fallback: assume first space separates filename and caption
                parts = line.split(' ', 1)
                filename = parts[0]
...
def evaluate():
    ...
    if total_queries == 0:
        print("No queries found. Please check your captions.txt format.")
        return
